...
def plot_SiF(df_both, wave_info, date, age, cell_num):
    ...
    if SiFs.empty:
        ...
        fig, ax = plt.subplots()
        fig.set_size_inches(11.7, 8.27)
        _ = plt.plot(SiF_mean, color = 'k', label = 'SiF_mean', linewidth = 0.7)

        #plot attributes
        _ = plt.legend(loc = 4, fontsize = 'x-small', ncol = 3)
        _ = plt.title(date + ': ' + age + ' cell ' + str(cell_num) + ': silent fibers')
        _ = plt.xlabel('seconds')
        _ = plt.ylabel('pA')

        #plot individual SiFs
